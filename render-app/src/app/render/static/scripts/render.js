...
// var CANVAS_HEIGHT = window.innerWidth * 0.5;
...
//         }
//     }
//     else if (event.key == "d") { // d key, delete all
...
//     }
//     else if (event.key == "l") { // d key, load new
//         update_JsonNameList_and_render("complex_Pipe");
//         //  loadJsons();
...
function add2scene(json, bbox = false) {
    ...
    if (bbox) {
        ...
        var material = new THREE.LineBasicMaterial({ color: 0xFF0000 });// red line color 

        var wireframe = new THREE.LineSegments(geometry, material);

        scene.add(wireframe);
        liste.push({ "uuidmesh": wireframe.uuid, "graphUri": json.uuid });
    } else {
        material = new THREE.MeshPhongMaterial({ color: 0xa5a5a5, specular: 0xffffff, shininess: 0.9, });

        mesh = new THREE.Mesh(geometry, material);
        // add to liste for later
        liste.push({ "uuidmesh": mesh.uuid, "graphUri": json.uuid });
