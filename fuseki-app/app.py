...
api = Api(app=app, version="0.1_B", title="fuseki-app: API documentation",
...
model_sparql = api.model("sparql_model", {
    "SparqlString": fields.String(required=True, description='make a sparql query', example="SELECT ?g ?s ?p ?o WHERE { { ?s ?p ?o }UNION{ GRAPH ?g { ?s ?p ?o } } }")
})

model_upload = api.model("upload_model", {
    "data": fields.String(required=True, description='upload a file', example="enter triples here!"),
})

model_insert = api.model("insert_model", {
    "SparqlString": fields.String(required=True, description='make a sparql insert query',
                                  example="INSERT DATA { GRAPH <http://test/test> { <http://test/testtest> <http://test.de/hatEintrag> 'Das ist mein Testeintrag' . } }")
})

model_delete = api.model("delete_model", {
    "SparqlString": fields.String(required=True, description='make a sparql delete query',
...
class sparql(Resource):
    ...
    def post(self):
        ...
        r = requests.post(url, data=payload, headers={
                          'content-type': 'application/sparql-query'}, proxies=proxies)
        return r.json()
...
class uploadData(Resource):
    ...
    def post(self):
        ...
        insertString = "INSERT DATA { GRAPH <http://%s> { " % give_me_new_IDs(1)[
            0]
        for row in qres:
            for col in row:
        ...
        r = requests.post(url, data=insertString, headers={
                          'content-type': 'application/sparql-update'}, auth=("admin", "admin"), proxies=proxies)
        return r.text
...
class insData(Resource):
    ...
    def post(self):
        ...
        payload = body["SparqlString"]
        r = requests.post(url, data=payload, headers={
                          'content-type': 'application/sparql-update'}, proxies=proxies, auth=("admin", "admin"))
