...
# sys.path.append("C:/Users/Nothsteinchr/.conda/envs/py27env/Lib/site-packages")
...
def createList(graph, ListItems):
    ...
    Triples.append([ListNode, RDF.type, oop.List])
    ItemName = BNode()
    BlankNodes.append(ItemName)
    graph.add((ListNode, oop.hasListContent, ItemName))
    Triples.append([ListNode, oop.hasListContent, ItemName])

    for i1 in range(len(ListItems) - 1):
        Item = ListItems[i1]
        NextItem = BNode()
        BlankNodes.append(NextItem)
        if type(Item) == rdflib.term.URIRef:
            graph.add((ItemName, RDF.first, Item))
            Triples.append([ItemName, RDF.first, Item])

        else:
            graph.add((ItemName, RDF.first, Literal(Item)))
            Triples.append([ItemName, RDF.first, Literal(Item)])
        graph.add((ItemName, RDF.rest, NextItem))
        Triples.append([ItemName, RDF.rest, NextItem])

        ItemName = NextItem

    if type(ListItems[-1]) == rdflib.term.URIRef:
        graph.add((NextItem, RDF.first, ListItems[-1]))
        Triples.append([NextItem, RDF.first, ListItems[-1]])
    else:
        graph.add((NextItem, RDF.first, Literal(ListItems[-1])))
        Triples.append([NextItem, RDF.first, Literal(ListItems[-1])])

    graph.add((NextItem, RDF.rest, RDF.nil))
    Triples.append([NextItem, RDF.rest, RDF.nil])
...
dataDict = {"Namespaces": {"BIM": "https://projekt-scope.de/ontologies/BIM#", "Revit": "https://projekt-scope.de/software/Revit#",
                           "RevitAPI": "https://projekt-scope.de/software/RevitAPI#", "oop": "https://projekt-scope.de/ontologies/oop#", "bot": "https://w3id.org/bot#"}}
Triples = []
BlankNodes = []
...
Triples.append([ProjID, RevitAPI.hasName, Literal(ProjectName)])
g.add((ProjID, RDF.type, bot.Building))
Triples.append([ProjID, RDF.type, bot.Building])
...
def addTypeToGraph(builtInCat, fNode, famID, allTypes):
    ...
    Triples.append(
        [FamUriRefNode, RevitAPI.fromBuiltInCategory, Revit[builtInCat]])
    g.add((fNode, RevitAPI.hasRevitType, FamUriRefNode))
    Triples.append([fNode, RevitAPI.hasRevitType, FamUriRefNode])
    g.add((FamUriRefNode, RDF.type, Revit.Type))
    Triples.append([FamUriRefNode, RDF.type, Revit.Type])
    g.add((FamUriRefNode, RevitAPI.hasName, Literal(getFamID.get("famName"))))
    Triples.append([FamUriRefNode, RevitAPI.hasName,
                    Literal(getFamID.get("famName"))])
    for entry in getFamID.get("paramList"):
        anode = BNode()
        BlankNodes.append(anode)
        g.add((FamUriRefNode, RevitAPI.hasParameter, anode))
        Triples.append([FamUriRefNode, RevitAPI.hasParameter, anode])
        g.add((anode, RevitAPI.hasName, Literal(toUTF8(entry[0]))))
        Triples.append([anode, RevitAPI.hasName, Literal(toUTF8(entry[0]))])
        g.add((anode, RevitAPI.hasValue, Literal(entry[1])))
        Triples.append([anode, RevitAPI.hasValue, Literal(entry[1])])
...
for builtInCat in builtInCats:
    ...
    for inst in inst_collector:
        ...
        Triples.append([fNode, RevitAPI.hasName, Literal(toUTF8(inst.Name))])
        g.add((fNode, RevitAPI.hasRevitID, Literal(inst.Id.IntegerValue)))
        Triples.append([fNode, RevitAPI.hasRevitID,
                        Literal(inst.Id.IntegerValue)])

        addTypeToGraph(builtInCat, fNode, famID, allTypes)

        g.add((fNode, RDF.type, bot.Element))
        Triples.append([fNode, RDF.type, bot.Element])
        g.add((ProjID, bot.hasElement, fNode))
        Triples.append([ProjID, bot.hasElement, fNode])
        ...
        for entry in inst.GetOrderedParameters():
            ...
            Triples.append([fNode, RevitAPI.hasParameter, anode])
            BlankNodes.append(anode)

            g.add((anode, RevitAPI.hasName, Literal(toUTF8(parName1))))
            Triples.append(
                [anode, RevitAPI.hasName, Literal(toUTF8(parName1))])
            g.add((anode, RevitAPI.hasValue, Literal(paramValue)))
            Triples.append([anode, RevitAPI.hasValue, Literal(paramValue)])
        ...
        if builtInCat == "OST_Walls":
            ...
            Triples.append(
                [fNode, RevitAPI.hasFurtherGeomParams, BIM[geomPar]])
            # Type of Line
            g.add((BIM[geomPar], RevitAPI.isBasedOn, Literal(isType)))
            Triples.append([BIM[geomPar], RevitAPI.isBasedOn, Literal(isType)])
            ...
            if str(isType) == "Line":
                ...
                Triples.append([BIM[geomPar], RevitAPI.hasDirection, dirNode])
                ...
                Triples.append(
                    [BIM[geomPar], RevitAPI.hasExactStartLoc, startLocNode])
                ...
                Triples.append(
                    [BIM[geomPar], RevitAPI.hasExactEndLoc, endLocNode])
                ...
                if len(inserts) == 0:
                    ...
                    for openingsInst in inserts:
                        ...
                        Triples.append([BIM[cutOutNode[0]],
                                        RevitAPI.CutInst,
                                        Literal(int(elem1.Id.IntegerValue))])
                        instLocPoint = [
        ...
        if builtInCat == "OST_StructuralColumns":
            ...
            try:
                ...
                for i, curve in enumerate(in2.Curves):
                    ...
                    Triples.append(
                        [BIM[geomPar], RevitAPI.hasCurve, BIM[curveNode]])
                    g.add((BIM[curveNode], BIM.hasCurveNr, Literal(i)))
                    Triples.append(
                        [BIM[curveNode], BIM.hasCurveNr, Literal(i)])
                    startLocNode = createList(g, startP)
                    g.add(
                    ...
                    Triples.append(
                        [BIM[curveNode], RevitAPI.isOfType, Literal(CuType)])
                    if CuType == "Arc":
                        ...
                        Triples.append(
                            [BIM[curveNode], RevitAPI.hasCenter, centerNode])
                        g.add(
                            (BIM[curveNode], RevitAPI.hasRadius,
                             Literal(curve.Radius))
                        )
                        Triples.append([BIM[curveNode], RevitAPI.hasRadius,
                                        Literal(curve.Radius)])
                    ...
                    Triples.append(
                        [BIM[geomPar], RevitAPI.hasLength, Literal(in1.GetDrivingCurve().Length)])
                ...
                Triples.append(
                    [BIM[geomPar], RevitAPI.hasLocation, LocPointNode])
            except:
        ...
        if builtInCat == "OST_Floors" or builtInCat == "OST_StructuralFoundation":
            ...
            if inst.GetType().Name != "FamilyInstance":
                ...
                if len(ListOfListNodeArr) == 1:
                    ...
                    Triples.append(
                        [fNode, RevitAPI.hasFurtherGeomParams, furParNode])
                    g.add((furParNode, RevitAPI.hasPolyline,
                           ListOfListNodeArr[0]))
                    Triples.append([furParNode, RevitAPI.hasPolyline,
                                    ListOfListNodeArr[0]])
                    ...
                    Triples.append(
                        [furParNode, RevitAPI.hasPolyline, listOfArrays])
            else:
                pass
...
dataDict["BlankNodes"] = BlankNodes
jsonContent = {"data": dataDict}
json_data = json.dumps(jsonContent)
conn = httplib.HTTPConnection(
    host="10.200.29.203", port="22633")
# params = urllib.urlencode(json_data)
# params = urllib.urlencode(jsonContent)
headers = {"Content-type": "application/json"}
conn.request("POST", "/revitAPI/fetchRevitData", json_data, headers)
